    ...
    async saveRoutine() {
      try {
        ...
        const routineData = {
          ...
          exercises: [this.newExercise, this.newExercise1]
        }
  ...
  <div id="mylutin_all">
    ...
    <div id="mylutin_content">
      ...
      <div class="routine-list">
        <h2>루틴 목록</h2>
        <ul>
          <!-- 요일별 루틴 목록을 반복문을 사용해 출력 -->
          <li v-for="(routines, day) in routinesByDay" :key="day">
            <p>
              {{ day }}요일 루틴:
              {{
                routines
                  .map((routine) => routine.exercises.join(', '))
                  .join(' ')
              }}
            </p>
          </li>
  ...
  methods: {
    ...
    async saveRoutine() {
      try {
        ...
        this.showInputs = false
        // 루틴 저장 후에 목록 다시 불러오기
        this.routinesByDay[this.routineData.day].push(routineData)
      } catch (error) {
        console.error('루틴을 저장하는 중 오류 발생:', error)
      }
    },
